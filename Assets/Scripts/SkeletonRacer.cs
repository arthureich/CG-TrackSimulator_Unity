...
public class SkeletonRacer : MonoBehaviour
{
    ...
    public RaceState raceState = RaceState.Ready;
    
    private float _timerState = 0f;
    private float _timeWhenGunFired = 0f;
    private float _delayToSet = 1.5f; 
    private float _delayToGun = 2.0f; 

    [Header("Ambiente")]
    public TrackManager trackManager;
    public int raiaAlvo = 3; 
    public float offsetLateral = -0.6f;
    private float _offsetLargada = 0f;
    ...
    public TipoDeProva provaSelecionada = TipoDeProva.Sprint_100m;

    public float distanciaTotalDaProva = 100f;

    [Header("Controles")]
    public bool modoAutomatico = false;
    [Range(0.1f, 1.5f)] public float ritmoAutomatico = 0.95f;
    public bool usarAutoClick = false; 
    [Range(1f, 15f)] public float clicksPorSegundo = 9f; 
    private float _timerAutoClick = 0f;
    public float tempoReacaoAuto = 0.14f;

    [Header("Física")]
    public float velocidadeMaximaCap = 12.5f; 
    public float forcaBaseClique = 1.0f;      
    public float arrastoBase = 1.0f;          
    public float resistenciaAoLimite = 0.9f; 
    public float potenciaCurvaForca = 1.0f; 
    public float limiarArrancada = 5.0f; 
    public float forcaInicialMinima = 0.5f;
    public float arrastoDinamicoLinear = 0.0f; 
    public float arrastoDinamicoQuadratico = 0.0f; 

    [Header("Stamina")]
    public bool usarFadiga = true;
    public float staminaConsumoBase = 0f;      
    public float staminaConsumoLinear = 0f;    
    public float staminaConsumoCubico = 0f;    
    public float staminaConsumoExcesso = 0f;   
    [Range(0, 100)] public float stamina = 100f;
    public float targetPace = 0f;
    public float taxaRecuperacao = 5.0f;       
    public float limiarFadiga = 30f;           
    public float penalidadeFadigaMin = 0.25f;  
    public bool fadigaQuadratica = false;
    public bool fadigaDupla = false;

    [Header("Revezamento")]
    public SkeletonRacer colegaAnterior; 
    public float distanciaGatilho;
    public int numeroPerna = 1; 

    // --- ESTADO PÚBLICO ---
    public float velocidadeAtual = 0f;
    public float distanciaPercorridaTotal = 0f;
    public float tempoCorrida = 0f;
    public float tempoReacaoReal = 0f;
    [Header("Cinematico")]
    public float tempoAlvo = 0f; 
    public Color corForcada = Color.clear;
    ...
    public Slider uiVelocidade;
    public Slider uiFadiga;
    public Image uiCorFadiga;
    ...
    { 
        ...
        {
            stamina = 100f;
            if (raceState == RaceState.Ready) PosicionarNaPista();
    ...
    void Update()
    {
        ...
        if (raceState == RaceState.Running) {
            ...
            {
                float limitePerna = (numeroPerna * 100f) + 10f; 
        ...
        {
            velocidadeAtual = Mathf.MoveTowards(velocidadeAtual, 0f, Time.deltaTime * 4f);
    ...
    void ProcessarReacao()
    {
        ...
        else if (!isBot) 
        {
            ...
            { 
                ...
                {
                    Debug.Log("QUEIMOU LARGADA! (Muito rápido)");
    ...
    {
        if (trackManager == null) return _offsetLargada + (90f * (leg - 1));
        float baseStart100 = trackManager.GetPontoDeLargada(100f, 1);
        baseStart100 = (raiaAlvo-1) * 7.04f + ((100*leg)-10f); 
        return baseStart100;
    }
