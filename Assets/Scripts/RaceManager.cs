...
public class RaceManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (FindObjectOfType<SkeletonRacer>()) 
        {
            _focoAtualRacer = FindObjectOfType<SkeletonRacer>();
            _cameraDummyTarget.position = _focoAtualRacer.transform.position;
        }
    }
    ...
    void Update()
    {
        if (_cameraDummyTarget != null && _focoAtualRacer != null)
        {
            ...
            _cameraDummyTarget.rotation = Quaternion.Lerp(_cameraDummyTarget.rotation, _focoAtualRacer.transform.rotation, Time.deltaTime * 5f);
        }
    }
            ...
            for (int perna = 1; perna <= 4; perna++)
            {
                ...
                if (perna == 1)
                {
                    ...
                    if (ehTimeDoJogador) 
                    {
                        ...
                        if(_cameraDummyTarget) _cameraDummyTarget.position = novoCorredor.transform.position;
    ...
    void AtualizarTelaResultados()
    {
        ...
        if (GameSettings.provaSelecionada == SkeletonRacer.TipoDeProva.Revezamento_4x100m)
        {
            listaFiltrada = classificados.Where(r => r.nomeCorredor.Contains("(Final)")).ToList();
        }
