...
public class HUDController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Se o HUD sumir porque o corredor anterior terminou, forçamos aparecer se o novo estiver correndo
        if (_root != null)
        {
            if (_player.raceState == SkeletonRacer.RaceState.Finished || _player.raceState == SkeletonRacer.RaceState.FalseStart)
            {
                // Só esconde se REALMENTE acabou (não é revezamento ou é o ultimo)
                // Mas deixamos o RaceManager controlar o fim do jogo, aqui apenas escondemos se necessário
                // _root.style.display = DisplayStyle.None; 
                // (Comentei a linha acima para a UI não piscar/sumir na troca, ou você pode refinar a lógica)
            }
            else
            {
